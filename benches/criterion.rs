...
impl Camera {
    ...
    pub fn generate_rays(&self, viewport_width_px: u32, viewport_height_px: u32, region: &Rect, rays: &mut Vec<Ray>) {
        ...
        let vp_half_height = vp_half_width * ((viewport_height_px as f32 - 1.) / (viewport_width_px as f32 - 1.));
...
fn camera_benchmark(c: &mut Criterion) {
    let camera = Camera::new( Vec3::new(0., 0., 0.), Vec3::new(0., 0., 1.), Vec3::new(0., 1., 0.), 90.);
    let region = Rect { x: 0, y: 0, width: 1920, height: 1080 };
    let region2 = Rect { x: 0, y: 0, width: 32, height: 32 };

    let mut rays: Vec<Ray> = Vec::new();
    c.bench_function("generate_camera_rays_v2", |b| b.iter(|| camera.generate_rays(1920, 1080, &region, &mut rays)));
    c.bench_function("generate_camera_rays_v2_32x32", |b| b.iter(|| camera.generate_rays(1920, 1080, &region2, &mut rays)));
}
...
fn rand_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("rand_f32", |b| b.iter(|| nums.iter_mut().for_each(|n| *n = rng.random())));
}
...
fn cell_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("vec_cell", |b| b.iter(|| vec_cell.iter().for_each(|n| n.update(|x| x + 1.))));
}
